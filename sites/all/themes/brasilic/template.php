...
// Add Zen Tabs styles
if (theme_get_setting('brasilic_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'brasilic') .'/css/base/tabs.css');
}

function brasilic_preprocess_html(&$vars) {
  global $user, $language;

  // Add role name classes (to allow css based show for admin/hidden from user)
  foreach ($user->roles as $role){
    $vars['classes_array'][] = 'role-' . brasilic_id_safe($role);
  }
  ...
  if (!$vars['is_front']) {
    ...
    $vars['classes_array'][] = brasilic_id_safe('page-'. $path);
    $vars['classes_array'][] = brasilic_id_safe('section-'. $section);
  ...
  // Add IE classes.
  if (theme_get_setting('brasilic_ie_enabled')) {
    $brasilic_ie_enabled_versions = theme_get_setting('brasilic_ie_enabled_versions');
    if (in_array('ie8', $brasilic_ie_enabled_versions, TRUE)) {
      drupal_add_css(path_to_theme() . '/css/base/ie8.css', array('group' => CSS_THEME, 'browsers' => array('IE' => 'IE 8', '!IE' => FALSE), 'preprocess' => FALSE));
      drupal_add_js(path_to_theme() . '/js/selectivizr-min.js');
    }
    if (in_array('ie9', $brasilic_ie_enabled_versions, TRUE)) {
      drupal_add_css(path_to_theme() . '/css/base/ie9.css', array('group' => CSS_THEME, 'browsers' => array('IE' => 'IE 9', '!IE' => FALSE), 'preprocess' => FALSE));
    }
    if (in_array('ie10', $brasilic_ie_enabled_versions, TRUE)) {
      drupal_add_css(path_to_theme() . '/css/base/ie10.css', array('group' => CSS_THEME, 'browsers' => array('IE' => 'IE 10', '!IE' => FALSE), 'preprocess' => FALSE));
    }
  }

}

function brasilic_preprocess_page(&$vars, $hook) {
...
function brasilic_preprocess_node(&$vars) {
  ...
  // Merge first/last class (from brasilic_preprocess_page) into classes array of current node object.
  $node = $vars['node'];
...
function brasilic_preprocess_block(&$vars, $hook) {
  // Add a striping class.
 ...
 */
function brasilic_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];
  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('brasilic_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('brasilic_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('brasilic_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('brasilic_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('brasilic_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
 ...
 */
function brasilic_id_safe($string) {
 ...
 *
 */
function brasilic_menu_link(array $variables) {
  ...
  $element['#attributes']['class'][] = brasilic_id_safe($element['#title']);
 ...
 */
function brasilic_preprocess_menu_local_task(&$variables) {
 ...
 */
function brasilic_menu_local_tasks(&$variables) {
  $output = '';
